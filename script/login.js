...
async function handleSuccessfulLogin(user) {
  ...
  localStorage.setItem("alreadyLoggin", false);
...
async function rememberMeLogin(name, userID) {
  ...
  sessionStorage.setItem("remember_me", getRememberMe());
...
/**
 * fetches the necessary login data from the session storage
 * @returns {any} - data from session storage
 */
...
/**
 * Inserts the necessary login data into the input fields
 */
function initRememberMe() {
  let data = sessionStorage.getItem("remember_me");
  if (data !== null) {
    let value = data.split(", ");
    document.querySelector("#email_input").value = value[0];
    document.querySelector("#password_input").value = value[1];
  }
}