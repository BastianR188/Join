...
function disclaimer() {
  if (localStorage.getItem("alreadyLoggin")) { return }
  if (sessionStorage.getItem("remember_me")) { return }
  linkToIndex();
  navdisabled();
}
...
/**
 * changes the navigation back to the index
 * @returns {any}
 */
 ...
 * @returns {any}
 */
function checkCookieValueInContacts(url) {
  let local = localStorage.getItem("alreadyLoggin");
  let session = sessionStorage.getItem("remember_me");
  if (local === null) {
    if (session === null) { window.location.href = url }
  }
}
window.onpageshow = function (event) {
  if (event.persisted) {
    // Die Seite wurde aus dem Cache geladen, nachdem der Benutzer zurückgegangen ist
    location.reload();
  }
};